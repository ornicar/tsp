...
public:
    ...
    GrahamScan( size_t n, Point * _points )
        : N( n ),
          points( _points )
    {
    ...
    void debug()
    {
        ...
        {
            qDebug("%1.15f, %1.15f", lower_hull[i].first, lower_hull[i].second);
        }
        qDebug("%s", "upper");
        for(i=0; i<upper_hull.size(); i++)
        {
            qDebug("%1.15f, %1.15f", upper_hull[i].first, upper_hull[i].second);
        }
    }
...
//        }
    ...
    int findIndexByCoords(double x, double y)
    {
        ...
        qDebug("Impossible index for x=%1.15f, y=%1.15f", x, y);
